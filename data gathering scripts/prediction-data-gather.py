...
OUTPUT_FILE = "crop_drought_resistance_prediction.csv"
...
def create_prediction_dataset():
    ...
    df['ZmDREB2A'] = np.random.choice([1, 0], len(df), p=[0.4, 0.6])
    ...
    print(f"Prediction dataset complete! {len(df)} rows, {len(df.columns)} columns saved to {OUTPUT_FILE}")
